...
(function(){
  ...
  function injectPhysicsStyles(){
    ...
    style.textContent=`
      .physics-task-list{display:grid;gap:14px;margin-top:8px}
      .physics-period{background:#f8fbff;border:1px solid #dbeafe;border-radius:18px;padding:12px}
      .physics-period-title{display:flex;align-items:center;justify-content:space-between;gap:10px;color:#0f3f86;font-weight:950;margin-bottom:10px}
      .physics-task-card{background:#fff;border:1px solid #dbeafe;border-radius:16px;padding:12px;margin-top:10px;box-shadow:0 8px 20px rgba(15,63,134,.05)}
      .physics-task-head{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:8px}
      .physics-task-title{font-size:16px;font-weight:950;color:#102a56;line-height:1.35}
      .physics-time-badge{white-space:nowrap;background:#eff6ff;color:#1d4ed8;border:1px solid #bfdbfe;border-radius:999px;padding:6px 10px;font-weight:950;font-size:12px}
      .physics-task-meta{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:8px;margin:8px 0}
      .physics-task-meta span{background:#f0f7ff;border:1px solid #dbeafe;border-radius:12px;padding:7px 8px;font-size:12px;color:#1e3a8a;font-weight:850}
      .physics-pages{display:flex;flex-wrap:wrap;gap:7px;margin:8px 0}
      .physics-page-chip{display:inline-flex;align-items:center;gap:5px;border:1px solid #bfdbfe;background:#fff;color:#0f3f86;border-radius:999px;padding:6px 9px;font-size:12px;font-weight:850;text-decoration:none}
      .physics-outline{background:#fffaf0;border:1px solid #fdecc8;color:#7c4a03;border-radius:12px;padding:9px 10px;font-size:13px;margin-top:8px}
      .physics-task-actions{display:flex;flex-wrap:wrap;gap:8px;margin-top:10px}.physics-task-actions button{min-height:36px;padding:8px 11px;border-radius:12px;font-size:13px}
      .physics-compact-notice{margin-top:10px;background:#f0f7ff;border:1px solid #bfdbfe;border-radius:16px;padding:11px 12px;color:#1e40af;font-size:13px}
      @media(max-width:760px){.physics-task-head{display:grid}.physics-time-badge{justify-self:start}.physics-task-meta{grid-template-columns:1fr 1fr}.physics-task-actions{display:grid;grid-template-columns:1fr 1fr}.physics-task-actions button{width:100%}}
      @media(max-width:420px){.physics-task-meta,.physics-task-actions{grid-template-columns:1fr}.physics-page-chip{width:100%;justify-content:center}}
    `;
    document.head.appendChild(style);
  }
  function physicsPageLinks(task){
    if(!task.pages.length) return '<div class="physics-pages"><span class="physics-page-chip">无新视频：只做公式、模型和题型训练</span></div>';
    return `<div class="physics-pages">${task.pages.map(p=>`<a class="physics-page-chip" href="${p.url}" target="_blank" rel="noopener">P${p.part}｜${esc(p.title)}｜${fmt(p.durationSeconds)}</a>`).join("")}</div>`;
  }
  function physicsTaskCard(day,task,index){
    return `<div class="physics-task-card"><div class="physics-task-head"><div class="physics-task-title">${esc(task.label)}</div><div class="physics-time-badge">${esc(task.start)}—${esc(task.end)}｜${fmt(task.totalSeconds)}</div></div><div class="physics-task-meta"><span>原视频：${fmt(task.originalSeconds)}</span><span>建议倍速：${task.pages.length?task.speed+"x":"无"}</span><span>观看约：${fmt(task.watchSeconds)}</span><span>整理/做题：${fmt(task.notePracticeSeconds)}</span></div>${physicsPageLinks(task)}<div class="physics-outline">${esc(task.outline)}</div><div class="physics-task-actions">${task.pages.length?`<button type="button" data-open-physics-page="${day.date}|${index}|0">打开本任务第一个视频</button>`:""}<button type="button" data-mark-physics-task="${day.date}|${index}">这个任务完成</button></div></div>`;
  }
  function physicsTimeGridHtml(day){
    ...
    return `<div class="physics-task-list">${periods.map(period=>`<section class="physics-period"><div class="physics-period-title"><span>${period.label}</span><span>${period.tasks.length}个任务</span></div>${period.tasks.map((task)=>physicsTaskCard(day,task,day.physicsTasks.indexOf(task))).join("")}</section>`).join("")}</div>`;
  }
